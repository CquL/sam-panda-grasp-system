...
import time  # 【新增】用于控制循环频率，防止CPU占满
...
class GraspGeneratorNode:
    def __init__(self):
        ...
        # 建议 queue_size 稍微给大一点点，slop 1.0s 对于 inference 较慢的情况是合理的
    ...
    def callback(self, yolo_msg, pc_msg):
        # 【新增】如果上一帧还在处理（例如GPU推理慢），直接丢弃当前帧，防止堆积
        ...
        try:
            ...
            rospy.logerr(f"GraspNet 处理发生异常: {e}")
            import traceback
            traceback.print_exc()
        
        finally:
            # 【重要】无论成功还是失败，最后一定要释放锁，否则程序就彻底死锁了
            self.is_processing = False
    ...
    def run_visualization(self):
        ...
        while not rospy.is_shutdown():
            if self.has_new_data and self.vis_full_cloud is not None:
                ...
                vis.poll_events()
                vis.update_renderer()
                # 【新增】关键修改：没有新数据时睡眠 20ms，释放 CPU 资源给 ROS 回调
                time.sleep(0.02)
